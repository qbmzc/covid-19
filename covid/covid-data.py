...
directory = "/data/Space/covid/"  # 定义数据保存路径
...
# 获取中国历史数据及每日新增数据
chinaDayList = pd.DataFrame(data_3["chinaDayList"])  # 中国历史数据
...
chinaDayAddList = pd.DataFrame(data_3["chinaDayAddList"])  # 中国每日新增数据
...
hubei_notHhubei = data_3["dailyHistory"]  # 湖北与非湖北历史数据
for w in ["hubei", "notHubei"]:
    filename = directory + lastUpdateTime.split(' ')[0] + "_" + w + "_history_data.csv"
    get_data_1()

# 获取湖北省与非湖北每日新增数据
hubei_DayAdd = pd.DataFrame(data_3["dailyNewAddHistory"])  # 中国历史数据
...
# 获取武汉与非武汉每日新增数据
wuhan_DayAdd = data_3["wuhanDayList"]
...
global_data = data_2["foreignList"]
dictt = {"韩国": "Korea", "意大利": "Italy", "日本本土": "Japan", "美国": "United States"}
for j in dictt.keys():
